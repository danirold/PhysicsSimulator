...
class Viewer extends SimulationViewer {
	...
	protected void paintComponent(Graphics g) {
		...
		g.drawLine(_centerX, _centerY + 6, _centerX, _centerY - 6);
		g.drawLine(_centerX -6, _centerY, _centerX + 6, _centerY);
	...
	private void showHelp(Graphics2D g) {
        ...
        g.setColor(Color.RED);
		g.drawString("h: toggle help, v: toggle vectors, +: zoom-in, -: zoom-out, =: fit", 8, 15);
		g.drawString("g: show next group", 8, 33);
		g.drawString("l: move right, j: move left, i: move up, m: move down: k: reset", 8, 51);
		
		
		g.drawString("Scaling ratio: " + _scale, 8, 69);
		g.setColor(Color.BLUE);
		if (_selectedGroup == null) {
			g.drawString("Selected Group: all", 8, 87);
			
		}
		else g.drawString("Selected Group: " + _selectedGroup , 8, 87);
	...
	private void drawBodies(Graphics2D g) {
		for (Body b: _bodies) {
			if (isVisible(b)) {
				Shape circleShape = new Ellipse2D.Double(_centerX + b.getPosition().getX()/_scale, _centerY + b.getPosition().getY()/_scale, 10, 10);
				g.setColor(Color.BLACK);
				g.drawString(b.getId(), (int) (_centerX + b.getPosition().getX()/_scale), (int) (_centerY + b.getPosition().getY()/_scale - 5));
				g.setColor(_gColor.get(b.getgId()));
				g.draw(circleShape);
				g.fill(circleShape);
				
				if (_showVectors) {
					int x1 = (int) (_centerX + 5 + b.getPosition().getX()/_scale);
				    int y1 = (int) (_centerY + 5 + b.getPosition().getY()/_scale);
					Vector2D unitary_V = b.getVelocity().direction().scale(20.0);
					int x2 = (int) (x1 + unitary_V.getX());
					int y2 = (int) (y1 + unitary_V.getY());
					drawLineWithArrow(g, x1, y1, x2, y2, 5, 5,Color.GREEN, Color.GREEN);
					
					int x12 = (int) (_centerX + 5 + b.getPosition().getX()/_scale);
					int y12 = (int) (_centerY + 5 + b.getPosition().getY()/_scale);
					Vector2D unitary_F = b.getForce().direction().scale(20.0);
					int x22 = (int) (x12 + unitary_F.getX());
					int y22 = (int) (y12 + unitary_F.getY());
					
					drawLineWithArrow(g, x12, y12, x22, y22, 5, 5,Color.RED, Color.RED);
				}
			}
		}
		/*
